...
public class LevelComplatedScreen extends Group {
    ...
    public LevelComplatedScreen(float w, float h) {
        this.w = w;
        this.h = h;

        title = new Image(CrazyCrashCar.atlas.findRegion("level_completed"));
        title.setX(w - title.getWidth() / 2);
        title.setY(h);
        addActor(title);

        done = new ImageButton(
                new TextureRegionDrawable(CrazyCrashCar.atlas.findRegion("done_btn")),
                new TextureRegionDrawable(CrazyCrashCar.atlas.findRegion("done_btn_down")));
