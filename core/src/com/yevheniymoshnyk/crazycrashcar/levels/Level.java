...
import com.yevheniymoshnyk.crazycrashcar.controls.JumpGauge;
...
public class Level extends StageGame {
    ...
    public void setMusic(String name) {
        musicName = name;
        CrazyCrashCar.media.addMusic(name);
    }
    ...
    public void dispose() {
        if (musicName != null && musicHasLoaded) {
            CrazyCrashCar.media.stopMusic(musicName);
            CrazyCrashCar.media.removeMusic(musicName);
        }
        if (world != null) {
            world.dispose();
        }
    ...
    private ContactListener contactListener = new ContactListener() {
        ...
        public void beginContact(Contact contact) {
            ...
            if (bodyA == player.driver) {
                playerTouch(bodyA);
                return;
            }
            if (bodyB == player.driver) {
                playerTouch(bodyB);
                return;
            }

            if (bodyA == player.car) {
                UserData date = (UserData) bodyA.getUserData();
                if (date != null) {
                    if (date.name.equals("land")) {
                        player.touchGround();
                        return;
                    }
                }
            ...
            if (bodyB == player.car) {
                UserData date = (UserData) bodyB.getUserData();
                if (date != null) {
                    if (date.name.equals("land")) {
                        player.touchGround();
                        return;
    ...
    private void playMusic() {
        if (musicName != null && musicHasLoaded) {
            CrazyCrashCar.media.playMusic(musicName, true);
        }
    }
    ...
    private void createLands(MapObjects objects) {
        ...
        Array<Polygon> children;

        for (MapObject object : objects ) {
            if (object instanceof PolygonMapObject) {
                polygon = ((PolygonMapObject) object).getPolygon();
                scaleToWorld(polygon);
                children = getTriangles(polygon);
                addPolygonLand(children);
            } else {
    ...
    protected void playerTouch(Body body) {
        ...
        if (data != null) {
            if (data.equals("land") && !player.isHasDestroyed()) {
                if (player.getRotation() < -90 || player.getRotation() > 90) {
                    player.destroy();
                    CrazyCrashCar.media.playSound("crash.ogg");
                    levelFailed();
                } else {
                    player.touchGround();
                }
